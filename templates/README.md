...
Six multi-page HTML templates, one per business category, written to import
...
Then in the builder: **Dashboard → Start a new site → .zip…**, pick the
archive, and hit *Create*. The `Folder…` button works equally well if you
would rather point at the unpacked directory.

Every template imports with its stylesheet merged, its images resolved, and
all three pages created as real routes.
...
```bash
node templates/make-images.mjs
```

The images are flat gradients generated by a small hand-rolled PNG encoder
rather than stock photography: they ship in the repo, so they need to be
licence-free, and they need to be small, because every one becomes a base64
data URL on import and D1 caps a row at 2 MB. Swap them for real photography
from the builder's Media panel.

## Checking a template still imports cleanly

```bash
node templates/audit.mjs              # all templates
node templates/audit.mjs restaurant   # just one
```
...
controls. All six currently import with zero.
